...
class Superwar {
  ...
  fight = () => {
    ...
    let bothTeamScore = document.getElementById('score');
    bothTeamScore.innerHTML = '1 - 0';
  ...
  calculateScore = () => {
    ...
    // Type your code here
    let setWinner = this.winner();
    let score = { hero: 0, villain: 0 };

    this.players.forEach((player) => {
      const winnerType = player.type === 'hero' ? 'hero' : 'villain';
      player.win = setWinner[winnerType];
      score[winnerType] += player.wins;
    });
  ...
  winner = () => {
    const win = {hero: 0, villain: 0, draw: 0};
    const winner = this.checkWin();
    if (winner === "hero") win.hero += 1;
    else if (winner === "villain") win.villain += 1;
    else win.draw += 1;
    return win;
  }
  ...
  totalStrength = (type) => {
    ...
    // Type your code here
    let strength = {hero: 0, villain: 0};
    this.players.forEach((player)=>{
      let playerType = player.type === "hero" ? "hero" : "villain";
      strength[playerType] += player.strength;
    })
